...
export const authMiddleware = (
  ...
  // O tipo Request não tem a propriedade user, então fazemos um cast para any
  // Se tentarmos fazer req.user = ..., o TypeScript vai reclamar
  // Para "enganar" o TypeScript, usamos um type assertion (req as any)
  // Type Assertion força o compilador a tratar uma entidade como um tipo diferente
  // Isso diz: "confie em mim, trate req como any"
  ...
  // - Saber quem está logado. Exemplo: /me retorna os dados do usuário logado usando req.user.id.
  // - Verificar permissões. Exemplo: só um usuario com role: "admin" pode acessar certas rotas: if (req.user.role !== "admin") return res.status(403).json();
  // Para associar recursos ao usuário logado. Exemplo: criar um post automaticamente ligado ao req.user.id. Não precisa mandar userId nop body.
